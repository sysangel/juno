...
=== NOTES ===
This change consumes the frozen `PermissionPolicyOptions` and `Settings` seams without redefining them. In `cli.ts`, the single shared `createPermissionPolicy` call now forwards `settings.permissionMode` plus seeded `allow`/`deny` lists; because the same `policy` instance flows to both the executor and `SubagentDeps.policy`, subagents inherit the mode with no further wiring. In `selectors.ts`, `permissionMode` is added to both `StatusLineState` and the `selectStatusLine` context param as an optional passthrough field — the reducer does not own it, mirroring the existing `model`/`cwd`/`skills` pattern. `StatusLine.tsx` renders the chip only for non-default modes, using the `warning` theme token (per the brief's explicit note that the token is `'warning'`, not `'warn'`), keeping the default UI clean. The new test exercises pure passthrough, the acceptEdits render, and both suppression cases (explicit `default` and absent), feeding `permissionMode` directly through `selectStatusLine`'s context param so `app.tsx` is untouched.
```
