...
describe('runtime mode flips via setMode', () => {
  ...
  it('still prompts spawn_subagent after a deny-seed AND a runtime flip', () => {
    // Combined invariant: a seeded deny plus a runtime acceptEdits flip must not
    // open spawn_subagent — it is excluded from ACCEPT_EDITS_TOOLS by name.
    const policy = createPermissionPolicy({ deny: ['write_file:secret.txt'] });
    policy.setMode('acceptEdits');
    expect(policy.evaluate('spawn_subagent', { path: '' }, 'risky')).toBe('prompt');
  });
