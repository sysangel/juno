...
describe('mode-consumption seam (Wave 1B): input.mode is read at body construction, NO-OP today', () => {
  // The adapters route input.mode through a private applyMode hook that is
  // UNCONDITIONALLY in buildRequestBody's return path. Today it returns the
  // body unchanged. These tests prove the seam is on the hot path (it runs for
  // every turn) AND is a true no-op: supplying any mode produces a request body
  // byte-identical to the no-mode baseline. When Wave 2 makes applyMode do real
  // work, these tests will turn red — exactly the regression tripwire we want.
  ...
  async function captureBody(
    ...
    await drain(client, { ...baseInput, mode }, noTools);
    return captured[0]?.body;
  }

  it('OpenAI: plan/ultracode modes yield a body identical to the no-mode baseline', async () => {
    const deps = {
      provider: { baseUrl: 'https://api.openai.test/v1', apiKeyEnv: 'OPENAI_TEST_KEY' },
      env: { OPENAI_TEST_KEY: 'secret-openai-key' },
    };
    const baseline = await captureBody(openAIEntry(), deps, openaiChunks, undefined);
    const plan = await captureBody(openAIEntry(), deps, openaiChunks, 'plan');
    const ultra = await captureBody(openAIEntry(), deps, openaiChunks, 'ultracode');

    expect(baseline).toBeDefined();
    // applyMode ran (it's unconditional in the return path) and changed nothing.
    expect(plan).toEqual(baseline);
    expect(ultra).toEqual(baseline);
    // The mode value itself must never leak into the wire request.
    expect(Object.hasOwn(plan ?? {}, 'mode')).toBe(false);
  });

  it('OpenRouter: mode does not alter the body, incl. the no-train provider block', async () => {
    const deps = {
      provider: { baseUrl: 'https://openrouter.ai/api/v1', apiKeyEnv: 'OPENROUTER_TEST_KEY' },
      env: { OPENROUTER_TEST_KEY: 'secret-openrouter-key' },
    };
    const baseline = await captureBody(openRouterEntry(), deps, openaiChunks, undefined);
    const plan = await captureBody(openRouterEntry(), deps, openaiChunks, 'plan');

    expect(plan).toEqual(baseline);
    // The no-train block is still present and unchanged under a mode.
    expect(asObject(plan?.provider)).toEqual({ data_collection: 'deny', allow_fallbacks: true });
  });

  it('Anthropic: plan/ultracode modes yield a body identical to the no-mode baseline', async () => {
    const deps = {
      provider: { baseUrl: 'https://api.anthropic.test', apiKeyEnv: 'ANTHROPIC_TEST_KEY' },
      env: { ANTHROPIC_TEST_KEY: 'secret-anthropic-key' },
    };
    const baseline = await captureBody(anthropicEntry(), deps, anthropicChunks, undefined);
    const plan = await captureBody(anthropicEntry(), deps, anthropicChunks, 'plan');
    const ultra = await captureBody(anthropicEntry(), deps, anthropicChunks, 'ultracode');
