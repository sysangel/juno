...
function buildRequestBody(
  ...
  return applyMode(body, input.mode);
}

/**
 * NO-OP mode hook. Reads the turn's `mode` so adapters have a single, tested
 * seam to route execution mode into the request body. Today it returns `body`
 * UNCHANGED — it must NOT alter any request shape (Wave 1 = seam only). The
 * already-complete body (incl. the OpenRouter no-train block) passes through
 * verbatim.
 * TODO(Wave 2): map mode -> effort / reasoning budget here.
 * TODO(Wave 5): full plan-mode / effort plan wiring.
 */
function applyMode(body: JsonObject, _mode: TurnInput['mode']): JsonObject {
  return body;
}
