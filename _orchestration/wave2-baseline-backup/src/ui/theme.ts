...
//   modeBadge.{normal,plan,ultracode}            <- State['mode']
...
export interface Theme {
  ...
  // mode badge — keyed 1:1 to State['mode'] from W3
  modeBadge: {
    normal: Hex;
    plan: Hex;
    ultracode: Hex;
...
const MODE_BADGE_NAMES = [
  'normal',
  'plan',
  'ultracode',
] as const satisfies readonly ModeBadgeName[];
...
function isModeBadgeName(value: string): value is ModeBadgeName {
  return (MODE_BADGE_NAMES as readonly string[]).includes(value);
}
 ...
 * Resolves a dotted `FlatTokenName` ('text', 'modeBadge.plan', ...) to its Hex
...
export function token(name: FlatTokenName, depth?: ColorDepth): string {
  ...
  if (name.startsWith(MODE_BADGE_PREFIX)) {
    ...
    const hex = isModeBadgeName(modeName) ? theme.modeBadge[modeName] : theme.text;
