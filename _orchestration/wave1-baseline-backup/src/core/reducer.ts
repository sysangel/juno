 ...
 * (`user-submit`, `set-mode`, `cycle-mode`, `set-overlay`, `clear`).
 */
export type Action =
...
const MODE_ORDER: ReadonlyArray<State['mode']> = ['normal', 'plan', 'ultracode'];
...
export function reducer(state: State, action: Action): State {
  switch (action.t) {
    ...
    case 'clear':
      ...
      return { ...initialState(), mode: state.mode, tokens: state.tokens };
  }
}
