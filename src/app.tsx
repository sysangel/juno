...
export function systemPromptForProvider(
  ...
  const truthGuidance = 'Harness truthfulness: describe a review as independent or delegated only after actually using the available subagent tool and recording its completion. Juno verifies delegation only from recorded Agent, Task, spawn_agent, or spawn_subagent tool events; prose is not evidence and unsupported claims are visibly marked unverified. On raw-provider re-entry, a <juno-delegation-evidence> fact block reports the authoritative counts. A failed or completed command is terminal; only a managed process explicitly reporting status running may be described as still running.';
